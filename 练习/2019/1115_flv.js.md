...
flv.js 通过`flvjs`对象导出所有接口，该对象在全局上下文`window`中公开。

`flvjs`对象也可以通过require或ES6导入来访问。
...
根据`mediaDataSource`中指示的`type`字段创建一个播放器实例，并带有可选的`config`。

### MediaDataSource 媒体数据源
...
| `hasAudio?`        | `boolean`             | Indicates whether the stream has audio track 指示流是否有音频轨道 |
| `hasVideo?`        | `boolean`             | Indicates whether the stream has video track 指示流是否有音频轨道 |
| `duration?`        | `number`              | Indicates total media duration, in **milliseconds** |
| `filesize?`        | `number`              | Indicates total file size of media file, in bytes 指示媒体文件的总文件大小，以字节为单位 |
